...
func NewFileStream(f unsafe.Pointer) FileStream {
	return NewFileStreamWithMode(f, "a")
}

func NewFileStreamWithMode(f unsafe.Pointer, mode string) FileStream {
	cMode := C.CString(mode)
	defer C.free(unsafe.Pointer(cMode))
	s := C.g_mime_stream_file_new((*C.FILE)(f))
	fileStream := (*C.GMimeStreamFile)(unsafe.Pointer(s))
	defer unref(C.gpointer(fileStream))
	return CastFileStream(fileStream)
}

func NewFileStreamWithBounds(f unsafe.Pointer, start int64, end int64) FileStream {
	mode := C.CString("r")
	defer C.free(unsafe.Pointer(mode))
	sBound := C.g_mime_stream_file_new_with_bounds((*C.FILE)(f), (C.gint64)(start), (C.gint64)(end))
